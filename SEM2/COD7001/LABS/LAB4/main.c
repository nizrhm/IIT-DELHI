...
int main(int argc, char** argv) {
    if (argc < 2) {
        printf("Usage: %s <bytecode_file.bin>\n", argv[0]);
        return 1;
    }

    int program_size = 0;
    int* program = load_bytecode(argv[1], &program_size);
    if (!program) return 1;

    VM vm = { .pc = 0, .sp = -1, .rsp = -1, .running = true };

    printf("Starting VM execution for %s...\n", argv[1]);
    execute(&vm, program);

    if (vm.sp >= 0) {
        printf("Execution Finished. Final Result (Top of Stack): %d\n", vm.stack[vm.sp]);
    } else {
        printf("Execution Finished. Stack is empty.\n");
    }
