...
void initialize_logs() {
    ...
    health_log << "timestamp,server_ip,server_port,healthy,response_time_ms" << endl;
    metrics_log << "timestamp,total_requests,healthy_servers,algorithm" << endl;
    request_log << "timestamp,client_ip,server_ip,server_port,operation,filename" << endl;
    
    cout << "Log files initialized with timestamp: " << timestamp.str() << endl;
}

void log_health_check(const SimpleServer& server, bool healthy, long long response_time) {
...
void log_request(const string& client_ip, const SimpleServer& server, const string& operation, const string& filename) {
    lock_guard<mutex> lock(log_mutex);
    auto now = chrono::system_clock::now();
    auto timestamp = chrono::duration_cast<chrono::milliseconds>(now.time_since_epoch()).count();
    ...
    while (running) {
        {
            ...
            for (auto& server : servers) {
                ...
                if (sock < 0) {
                    ...
                    log_health_check(server, false, -1);
                    continue;
                }
                ...
                log_health_check(server, healthy, response_time);
                close(sock);
            }
        }
...
void handle_client(int client_sock) {
    ...
    // Log the request
    log_request(client_ip, *target, command, filename);
...
int main(int argc, char* argv[]) {
    ...
    cout << "  - Health checks: health_checks_*.log" << endl;
    cout << "  - Metrics: metrics_*.log" << endl;
    cout << "  - Requests: requests_*.log" << endl;
